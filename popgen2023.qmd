...
1.  Write a function (in script `model2.R`) that gets $N_e$ as input, creates a population, compiles a model, simulates a `ts`, runs `ts_afs()` for 10 samples, and returns the AFS vector.

2.  Find the $N_e$ value that will give the closest AFS to the observed one. For instance, you could:

    -   a ) Plot AFS for various $N_e$ values and eyeball $N_e$ value that looks correct judging from a plot _[easy]_

    -   b ) Simulate AFS in steps of possible $N_e$ values and find the [closest matching](https://en.wikipedia.org/wiki/Mean_squared_error) one _[harder]_
