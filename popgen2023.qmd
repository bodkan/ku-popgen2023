...
**Compute and plot AFS for various `Ne` values. Observe how (and why?) do they differ based on `Ne` you simulated.**
...
simulate_afs <- function(Ne) {
  ... AFS code taken from slides: population(), compile_model(), msprime() ...
  
  result <- ... compute AFS using ts_afs(), save it to `result` ...
...
::: fragment
When used in R, it should work like this:
...
Use `msprime()` to simulate a 50Mb tree sequence `ts` from your introgression model in `model1.R`.
