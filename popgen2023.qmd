...
## Complete single-locus simulation R script
...
::: aside
[`ex1.R` script on GitHub](https://bodkan.quarto.pub/2023-08-23-ku-popgen23-onepage/) one-page handouts for reference.
:::
...
::: aside
[`ex2.R` script on GitHub](https://bodkan.quarto.pub/2023-08-23-ku-popgen23-onepage/) one-page handouts for reference.
:::
...
::: aside
[`ex3_ex4.R` script on GitHub](https://bodkan.quarto.pub/2023-08-23-ku-popgen23-onepage/) one-page handouts for reference.
:::
...
First get a list of individuals in each population:
...
Then use that to compute diversity:
...
options(scipen=999)
pi <- ts_diversity(ts, sample_sets = samples)
as.data.frame(arrange(pi, diversity))
```
:::
...
Compute pairwise divergence using the same `samples`:
...
#| echo: false
div <- ts_divergence(ts, sample_sets = samples)
as.data.frame(arrange(div, divergence))
```
...
ts_f3(ts, B = samples["AFR"], B = samples["EUR"], C = "CHIMP_1")
```

```{r}
#| echo: false
as.data.frame(ts_f3(ts, B = samples["AFR"], C = samples["EUR"], A = "CHIMP_1"))
```
...
#| echo: false
# A = Africans, B = Neanderthals, C = outgroup
as.data.frame(ts_f3(ts, A = samples["AFR"], B = samples["NEA"], C = "CHIMP_1"))
```
...
ts_f3(ts, A = samples["EUR"], B = samples["NEA"], C = "CHIMP_1")
```

```{r}
#| echo: false
# A = Europeans, B = Neanderthals, C = outgroup
as.data.frame(ts_f3(ts, A = samples["EUR"], B = samples["NEA"], C = "CHIMP_1"))
```
...
`f4(AFR_1, X = AFR_2; NEA, CH)` is "almost zero":
...
```{r}
#| echo: false
as.data.frame(ts_f4(ts, W = "AFR_1", X = "AFR_2", Y = "NEA_1", Z = "CHIMP_1"))
```

<br>

`f4(AFR, X = EUR; NEA, CH)` is "significantly negative":
...
```{r}
#| echo: false
as.data.frame(ts_f4(ts, W = "AFR_1", X = "EUR_1", Y = "NEA_1", Z = "CHIMP_1"))
```
...
rbind(f4_afr, f4_eur) %>%
  ...
  geom_boxplot() + geom_jitter() + geom_hline(yintercept = 0, linetype = 2)
```
