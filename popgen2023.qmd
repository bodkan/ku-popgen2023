...
[a one-page version of the slides, exercise solutions, etc.]
...
In order to make sure you can do the exercises, you should log into your [online RStudio session](emily.popgen.dk:3838/) and run this in your R console:
...
If you want to do the exercises on your own laptop (if you potentially _can_, you definitely _should_), you need to do this first before running the above:
...
Go to `Tools` → `Global Options` in your RStudio and set the options like this.

Also happens on the [shared server](http://emily.popgen.dk:3838/)!
:::
...
::: aside
You can specify events in "units of years ago". Just program times in a "decreasing order" (7Mya → 6Mya → ..., as above).
:::
...
::: aside
See [`ex1.R` script on GitHub](https://github.com/bodkan/ku-popgen2023/blob/main/solutions/ex1.R) for a solution.
:::
...
1.  Write a function (in a new script `model2.R`) taking `Ne` as a parameter. It should create one population, compile a model, run `ts_afs()` for 10 samples, and return the AFS.

::: fragment
If we call it `simulate_afs()`, it should work like this:

```{r}
#| echo: false
simulate_afs <- function(Ne) {
  n <- 10 + 1 # 1 is for the fixed sites included by tskit
...
::: aside
[Don't panic---you can pretty much just tweak bits of my earlier example code.]
:::

## Exercise #2: hints

2.  Using your custom `simulate_afs()` function, find the value of `Ne` that will give the closest AFS to the observed AFS:
...
::: aside
See [`ex3_ex4.R` script on GitHub](https://github.com/bodkan/ku-popgen2023/blob/main/solutions/ex3_ex4.R) for a solution.
:::
...
::: aside
See [`ex5.R` script on GitHub](https://github.com/bodkan/ku-popgen2023/blob/main/solutions/ex5.R) for a solution.
:::
...
#| output-location: fragment
#| echo: false
ts_genotypes(ts) %>% as.data.frame() %>% head(10)
```
