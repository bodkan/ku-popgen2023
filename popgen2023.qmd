...
You know that the population had constant $N_e$ somewhere between 1000 and 30000 for the past 100,000 generations.
...
2.  Find `Ne` that will give the closest AFS to the observed AFS:
    ...
    -   _option #2_ [_harder_]: Simulate AFS vectors in steps of possible `Ne` (`lapply()`?), find the [closest](https://en.wikipedia.org/wiki/Mean_squared_error) AFS
:::
...
If we have populations `p1` and `p2`, we schedule a gene flow as:
...
Add archaic introgression to your `model1.R` script:
...
## Exercise #4 --- additional statistics
