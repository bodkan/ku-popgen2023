...
## Exercise #1 --- writing a _slendr_ model
...
## Workflows always follow this pattern

::: incremental
1. You define populations and compile a `model`
2. You simulate a tree-sequence from the model

   [in this session, always `ts <- msprime(model, ...)`]

3. Get samples (`ts_names(ts)` or individual names directly)

4. Compute statistic with something along the lines of `ts_<statistic>(ts, <sample names>)`
5. All tree-sequence statistic functions return a data frame
:::
...
- outgroup [`ts_f3()`](https://www.slendr.net/reference/ts_f4ratio.html#ref-examples) using CHIMP as the outgroup for different pairs of "A" and "B" populations<br>
...
**Do results match your expectation given your model?**
...
```{r}
#| echo: false
as.data.frame(ts_f3(ts, A = samples["AFR"], B = samples["EUR"], C = "CHIMP_1"))
```
...
ts_f3(ts, A = samples["AFR"], B = samples["NEA"], C = "CHIMP_1")
```
...
- new R package for demographic inference [_demografr_](http://github.com/bodkan/demografr)
